...
//        url.addQueryItem("start_message_id", QString::number(-1) );
    ...
    void vk_api::vk_send_message(int userID, QString messageText, QString attachment, QString lattitude, QString longtitude)
    {
...
//        }
        ...
        {
            url.addQueryItem("attachment", attachment);
        }

        if (!lattitude.isEmpty())
        {
            url.addQueryItem("lat", lattitude);
            url.addQueryItem("long", longtitude);
        }
        url.addQueryItem("v", _api_version );
...
//        url.addQueryItem("long", QString::number(qrand()) );
        ...
        qDebug() << "vk_api::vk_send_message : \t url:" << url;

        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_sendMessage);
    }

    void vk_api::vk_post_to_wall(int wallID, QString messageText, bool isCommunity, QString attachment,
                                 QString lattitude, QString longtitude, bool friends_only, bool from_group)
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "wall.post");
...
//        url.addQueryItem("long", QString::number(qrand()) );
        ...
        qDebug() << "vk_api::vk_post_to_wall : \t url:" << url;

        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_wallPost);
    }

    void vk_api::vk_get_wall_comments(int wallID, uint post_id, bool isCommunity)
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "wall.getComments");
        ...
        url.addQueryItem("post_id", QString::number(post_id));
        url.addQueryItem("need_likes", QString::number(1));
        // TODO :: Make proper
        url.addQueryItem("count", QString::number(100));
        url.addQueryItem("sort", "asc");
        url.addQueryItem("preview_length", QString::number(0));
        url.addQueryItem("extended", QString::number(1));
        url.addQueryItem("v", _api_version );

        qDebug() << "vk_api::vk_get_wall_comments : \t url:" << url;

        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_wallGetComments);
    }
...
//        url.addQueryItem("long", QString::number(qrand()) );
    ...
    {
        // Load the JSON data
        qDebug() << "Parsing repoly from LPS";
        JsonDataAccess jda;
//        if ( reply->error() == QNetworkReply::NoError )
        {
...
//                    m_chatModel->insert(entry);
    ...
    void vk_api::vk_send_photo(uint userID, QString photoPath)
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "photos.getMessagesUploadServer");
        QUrl url(_fetchUrl);
//        url.addQueryItem("user_ids", users.join(",") );
//        url.addQueryItem("fields", "photo_100,status,online,last_seen,screen_name" );
        url.addQueryItem("access_token", _token );
        ...
        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_getUploadServer);
        reply->setProperty("userID", userID);
        reply->setProperty("photoPath", photoPath);
    }

    void vk_api::vk_upload_photo(int userID, QString photoPath, QVariant &list)
    {
        QVariantMap map = list.value<QVariantMap>();
        qDebug() << "vk_api::vk_upload_photo : Received data for image upload: " << list;
        QString l_url = map.value("upload_url").toString();
        QString l_albumID = map.value("aid").toString();
        QString l_userID  = map.value("mid").toString();
        QUrl url(l_url);
//        url.addQueryItem()
        // Read file from device
        qDebug() << "Reading image file : " << photoPath;
        QFile* file = new QFile(photoPath);
        if (file->open(QIODevice::ReadOnly))
        {
//            QVariantMap request;
//            QByteArray blob = file.readAll();
//            request["photo"] = QVariant(blob);
//            request["access_token"] =_token;
//            request["v"] = _api_version;
//            JsonDataAccess jda;
//            QByteArray _jsonBuffer;
//            jda.saveToBuffer(QVariant(request), &_jsonBuffer);
//            url.addQueryItem("access_token", _token );
//            url.addQueryItem("v", _api_version );
            qDebug() << "Starting upload to:" << url;
//            networkRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
//            defaulting to application/x-www-form-urlencoded. Use QNetworkRequest::setHeader() to fix this problem.
            QNetworkRequest networkRequest = QNetworkRequest(url);
//            networkRequest.setHeader(QNetworkRequest::ContentTypeHeader, "multipart/form-data");
            QHttpMultiPart *multiPart = new QHttpMultiPart(QHttpMultiPart::FormDataType);
            QHttpPart imagePart;
            imagePart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"photo\"; filename=\"kartinka.png\""));
            imagePart.setHeader(QNetworkRequest::ContentLengthHeader, QVariant(file->size()));
...
//            imagePart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"image\""));
            ...
            QNetworkReply* reply = m_pNetAccessMngr->post(networkRequest, multiPart);
            reply->setProperty("requestType", _request_uploadToServer);
            reply->setProperty("userID", userID);
            reply->setProperty("uploadServer", l_url);
            multiPart->setParent(reply);
        }
        else
        {
            qDebug() << "Can't read file: " << photoPath << ". Error:" << file->error() << "," << file->errorString();
        }
    }

    void vk_api::vk_save_message_photo(QString uploadServer, int userID, QVariant& list)
    {
        QVariantMap map = list.value<QVariantMap>();
        QVariant photoReply = map.value("photo");
        qDebug() << "vk_save_message_photo :: Received photo reply:" << photoReply.toString();
        QString _fetchUrl = QString("%1%2").arg(_url, "photos.saveMessagesPhoto");
        QUrl url(_fetchUrl);
//        url.addQueryItem("user_ids", users.join(",") );
//        url.addQueryItem("fields", "photo_100,status,online,last_seen,screen_name" );
        url.addQueryItem("server", map.value("server").toString());
        url.addQueryItem("hash", map.value("hash").toString());
        url.addQueryItem("photo", photoReply.toString());
        url.addQueryItem("access_token", _token );
        url.addQueryItem("v", _api_version );
//        QVariantMap request;
//        QNetworkRequest networkRequest = QNetworkRequest(url);
...
//        JsonDataAccess jda;
//        QByteArray _jsonBuffer;
//        jda.saveToBuffer(QVariant(map), &_jsonBuffer);
//        QNetworkReply* reply = m_pNetAccessMngr->post(networkRequest, _jsonBuffer);
        reply->setProperty("requestType", _request_uploadSavePhoto);
        reply->setProperty("userID", userID);
//        reply->setProperty("photoPath", photoPath);

        // Sending message
//        vk_send_message(userID, list.toString());
    }

    void vk_api::vk_send_message_with_photo(int userID, QVariant& list)
    {
        QVariantList l2 = list.value<QVariantList>();
        qDebug() << "vk_api::vk_send_message_with_photo :: list:";
        foreach(const QVariant &v, l2 )
        {
            qDebug() << v;
        }
        QVariantMap map = list.value<QVariantList>().value(0).value<QVariantMap>();
        for(QVariantMap::const_iterator iter = map.begin(); iter != map.end(); ++iter) {
          qDebug() << iter.key() << iter.value();
        }

        qDebug() << "Ready to send message with attachment:" << map;
//        qDebug() << "list:" << list;
        QString l_attachment = QString("%1%2_%3").arg("photo", map.value("owner_id").toString(), map.value("id").toString());
//        photo|owner_id_media_id
        vk_send_message(userID, "", l_attachment);
    }
...
//        url.addQueryItem("fields", "photo_100" );
    ...
    void vk_api::vk_get_popular_music()
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "audio.getPopular");
        QUrl url(_fetchUrl);
    //        url.addQueryItem("user_ids", _userID );
    //        url.addQueryItem("fields", "photo_100" );
        url.addQueryItem("access_token", _token );
        ...
        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_audioGetPopular);
    }

    void vk_api::vk_get_recommended_music(uint trackID, uint ownerID)
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "audio.getRecommendations");
        QUrl url(_fetchUrl);
    //        url.addQueryItem("user_ids", _userID );
    //
        if (trackID != 0 && ownerID != 0)
        {
            url.addQueryItem("target_audio", QString("%1_%2").arg(QString::number(ownerID), QString::number(trackID)) );
        }
        url.addQueryItem("access_token", _token );
        url.addQueryItem("v", _api_version );
        qDebug() << url;
    ...
    void vk_api::vk_add_to_my_music(uint audioID, uint ownerID)
    {
            QString _fetchUrl = QString("%1%2").arg(_url, "audio.add");
            QUrl url(_fetchUrl);
            url.addQueryItem("audio_id", QString::number(audioID) );
            ...
            QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
            reply->setProperty("requestType", _request_audioAddToMyList);
    }

    void vk_api::vk_delete_from_my_music(uint audioID, uint ownerID)
    {
        QString _fetchUrl = QString("%1%2").arg(_url, "audio.delete");
        QUrl url(_fetchUrl);
        url.addQueryItem("audio_id", QString::number(audioID) );
        url.addQueryItem("owner_id", QString::number(ownerID) );
        url.addQueryItem("access_token", _token );
        ...
        QNetworkReply* reply = m_pNetAccessMngr->get(QNetworkRequest(url));
        reply->setProperty("requestType", _request_audioRemoveFromMyList);
    }
    ...
    void vk_api::networkReplyReceived(QNetworkReply *reply)
    {
        ...
        if ( (_errCode != QNetworkReply::NoError) && ( _errCode != QNetworkReply::UnknownContentError) )
        {
            ...
            qDebug() << "Network error occured:" << _errCode << ", desc:" << networkError;
            log_lps_error(reply);
        ...
        QVariant fullList = jda.loadFromBuffer(buffer);

        int requestType = reply->property("requestType").toInt();
        uint fromID = reply->property("userID").toUInt();
        QString uploadServer = reply->property("uploadServer").toString();
        switch (requestType) {
...
//                    emit chatUpdated(list);
            ...
            case _request_wallGetComments:
                    emit wallCommentsDownloaded(list); break;
            case _request_myMusic:
            case _request_audioGetPopular:
                    emit musicUpdated(list); break;
            case _request_audioAddToMyList:
            case _request_audioRemoveFromMyList:
                    break;  // No additional actions needed
            case _request_usersInfo:
                    emit usersInfoUpdated(list); break;
            case _request_registerDevice:
                    break;  // No additional actions
//          TODO :: Check if something additional needed, maybe refresh feed
            case _request_wallPost: break; // No additional actions, for now
            case _request_getUploadServer:
                    vk_upload_photo(reply->property("userID").toInt(), reply->property("photoPath").toString(), list);
                    break;
            case _request_uploadToServer:
                    qDebug() << "Before sending buffer:" << buffer;
                    vk_save_message_photo(uploadServer, reply->property("userID").toInt(), fullList);
                    break;
            case _request_uploadSavePhoto:
                    vk_send_message_with_photo(reply->property("userID").toInt(), list);
                    break;
            default:
